...
import Step1TireCar from './Step1TireCar';
import Step2Branch from '../../../../service/[serviceSlug]/booking/components/Step2Branch';
import Step3DateTime from '../../../../service/[serviceSlug]/booking/components/Step3DateTime';
...
interface TireBookingFlowProps {
  tire: Tire;
  locations: Location[];
  quantity: number;
...
const getSteps = (flowType: 'booking' | 'quotation') => {
  ...
  if (flowType === 'booking') {
    return [
      ...baseSteps,
      { number: 3, title: 'Date & Time', description: 'Pick installation appointment' },
...
export default function TireBookingFlow({ tire, locations, quantity, flowType = 'booking' }: TireBookingFlowProps) {
    ...
    try {
      const response = await fetch('/api/bookings', {
        ...
        body: JSON.stringify({
          ...
          services: `${tire.brand} ${tire.pattern} - ${tire.tireSize} (Qty: ${quantity})`,
